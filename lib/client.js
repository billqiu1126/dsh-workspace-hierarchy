		...
		function apply(ctx) {
			...
			const browserInjected = () => ({
				...
				deleteSession: async (sessionId) => {
					const session = ctx.sessions.binding(sessionId)?.session;
					if (session === void 0) throw new Error(`unknown session "${sessionId}"`);
					const result = await session.remote.commands.execute(sessionId, `/delete-session ${sessionId}`);
					if (!result.ok) throw new Error(`delete-session failed: ${result.error.message}`);
				},
				moveSession: async (sessionId, targetCwd) => {
					const session = ctx.sessions.binding(sessionId)?.session;
					if (session === void 0) throw new Error(`unknown session "${sessionId}"`);
					const result = await session.remote.commands.execute(sessionId, `/move-session ${sessionId} ${targetCwd}`);
