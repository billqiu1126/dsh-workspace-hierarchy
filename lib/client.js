		...
		function apply(ctx) {
			...
			const browserInjected = () => ({
				...
				deleteSession: async (sessionId) => {
					...
					if (currentId === void 0) throw new Error("open a session first, then delete");
					...
					const result = await session.remote.commands.execute(currentId, `/delete-session ${sessionId}`);
					if (!result.ok) throw new Error(`delete-session failed: ${result.error.message}`);
				},
				moveSession: async (sessionId, targetCwd) => {
					const currentId = ctx.sessions.list.getSnapshot().current;
					if (currentId === void 0) throw new Error("open a session first, then move");
					const session = ctx.sessions.binding(currentId)?.session;
					if (session === void 0) throw new Error("no session binding for the current session");
					const result = await session.remote.commands.execute(currentId, `/move-session ${sessionId} ${targetCwd}`);
					if (!result.ok) throw new Error(`move-session failed: ${result.error.message}`);
				},
