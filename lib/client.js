window.__ModuleLoader__.load({
	id: "@billqiu1126/dsh-workspace-hierarchy",
	factory: (require) => {
		var module = { exports: {} };
		...
		*/
		...
		/** Normalize a path to its non-empty segments (both separators accepted). */
		function pathSegments(path) {
			return path.replace(/[\\/]+$/, "").split(/[\\/]/).filter((segment) => segment !== "");
		}
		/** True when `child` is a strict descendant of `parent` (case-insensitive for Windows drives). */
		function isNestedPath(child, parent) {
			...
			for (let index = 0; index < parentSegments.length; index++) if (childSegments[index].toLowerCase() !== parentSegments[index].toLowerCase()) return false;
			return true;
		}
		/** Parent map: each workspace's deepest path-ancestor workspace (undefined for a root workspace). */
		function workspaceParentMap(workspaces) {
			const parents = /* @__PURE__ */ new Map();
			for (const workspace of workspaces) {
				let parent;
				let bestLength = -1;
				for (const candidate of workspaces) {
					if (candidate.workspaceId === workspace.workspaceId) continue;
					if (!isNestedPath(workspace.path, candidate.path)) continue;
					if (candidate.path.length > bestLength) {
						parent = candidate.workspaceId;
						bestLength = candidate.path.length;
					}
				}
				parents.set(workspace.workspaceId, parent);
			}
			return parents;
		}
		/** Depth-first pre-order of workspace ids: roots in domain order, children nested under their parent. */
		function workspaceHierarchyOrder(workspaces, parents) {
			const children = /* @__PURE__ */ new Map();
			const roots = [];
			for (const workspace of workspaces) {
				const parent = parents.get(workspace.workspaceId);
				if (parent === void 0) roots.push(workspace.workspaceId);
				else {
			...
			const ordered = [];
			const visit = (workspaceId) => {
				ordered.push(workspaceId);
				for (const child of children.get(workspaceId) ?? []) visit(child);
			};
			for (const workspaceId of roots) visit(workspaceId);
			return ordered;
		}
		/** Nesting depth of a workspace under its path ancestors. */
		function workspaceDepth(workspaceId, parents) {
			let depth = 0;
			let current = workspaceId;
			for (;;) {
				const parent = parents.get(current);
				if (parent === void 0) return depth;
				depth += 1;
				current = parent;
			}
		}
		...
		*/
		function deriveGroups(list, workspaces, archivedSessionIds, view) {
			...
			const parents = workspaceParentMap(workspaces);
			const orderedIds = workspaceHierarchyOrder(workspaces, parents);
			const isVisible = (workspaceId) => {
				let current = workspaceId;
				for (;;) {
					const parent = parents.get(current);
					if (parent === void 0) return true;
					if (!expandedGroups.has(parent)) return false;
					current = parent;
				}
			};
			const baseByKey = new Map(groupByWorkspace(list, workspaces, archived, view.ungroupedOrder).map((g) => [g.key, g]));
			const groups = [];
			const push = (g, depth) => {
			...
			for (const workspaceId of orderedIds) {
				if (!isVisible(workspaceId)) continue;
				const g = baseByKey.get(workspaceId);
				if (g !== void 0) push(g, workspaceDepth(workspaceId, parents));
			}
			const ungrouped = baseByKey.get("");
			if (ungrouped !== void 0) push(ungrouped, 0);
			return groups;
		}
		/**
		...
		*/
		...
		const tagId$2 = "@billqiu1126/dsh-workspace-hierarchy/Rows.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId$2) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "@billqiu1126/dsh-workspace-hierarchy";
		...
		*/
		function ProjectRowItem({ group, onToggle, onCreate, onAddSubWorkspace, actions, drag, t }) {
			const row = group;
			const label = row.workspaceId === void 0 ? t("group.ungrouped") : row.label;
			const active = group.expanded && group.containsCurrent;
			const [menuOpen, setMenuOpen] = (0, react.useState)(false);
			const [plusMenuOpen, setPlusMenuOpen] = (0, react.useState)(false);
			const plusMenuItems = row.workspaceId === void 0 ? [{
				id: "new-session",
				...
				id: "add-sub-workspace",
				label: t("menu.addSubWorkspace"),
				icon: (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconProjectAddOutline16, {})
			}];
					...
					(0, react_jsx_runtime.jsxs)("span", {
						...
						children: [actions !== void 0 && (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Menu, {
							...
							onSelect: (id) => {
								...
								else if (id === "add-sub-workspace" && onAddSubWorkspace !== void 0) onAddSubWorkspace();
							},
							portal: true,
							closeOnPointerLeave: true,
							anchor: (0, react_jsx_runtime.jsx)("button", {
			...
			return (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.HoverCard, {
				...
				disabled: menuOpen || plusMenuOpen,
				copyText: row.cwd,
				copyLabel: t("copy"),
		...
		*/
		function SessionNodeItem({ node, currentId, now, onOpen, onRename, onFork, onArchive, onDelete, onMove, drag, flat = false, t }) {
			const row = node;
		...
		const tagId$1 = "@billqiu1126/dsh-workspace-hierarchy/WorkspacePicker.module.css";
		...
		*/
		...
		const tagId = "@billqiu1126/dsh-workspace-hierarchy/WorkspaceBrowser.module.css";
		...
		*/
		...
		function SessionTree({ useSessions, startSession, open, forkSession, workspaces, archivedSessionIds, onRenameRequest, onDeleteRequest, onSessionRename, onSessionArchive, insertWorkspaceBefore, insertSessionBefore, orderBy, groupExpansion, setGroupExpanded, sessionOrderByAccount, sessionUpdatedAtByAccount, syncSessionOrderAccount, setSessionOrder, onAddSubWorkspace, onSessionDelete, onSessionMove, t }) {
			...
			const workspaceParents = (0, react.useMemo)(() => workspaceParentMap(workspaces), [workspaces]);
			const lastAutoExpandedCurrent = (0, react.useRef)(void 0);
			(0, react.useEffect)(() => {
				if (current === void 0 || currentGroup === void 0) return;
				// Reveal the ancestor chain exactly once per session switch. A manual
				// collapse must NOT be fought, so `groupExpansion` is intentionally
				// not a dependency: this runs only when the open session changes.
				if (lastAutoExpandedCurrent.current === current) return;
				lastAutoExpandedCurrent.current = current;
				const toExpand = [];
				let id = currentGroup;
				for (;;) {
					if (groupExpansion[id] !== true) toExpand.push(id);
					const parent = workspaceParents.get(id);
					if (parent === void 0) break;
					id = parent;
				}
				for (const key of toExpand) setGroupExpanded(key, true);
			}, [
				current,
				currentGroup,
				workspaceParents,
							...
							return (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									(0, react_jsx_runtime.jsx)(ProjectRowItem, {
										...
										onAddSubWorkspace: group.workspaceId === void 0 ? void 0 : onAddSubWorkspace,
										drag: workspaceDragProps,
										actions: group.workspaceId === void 0 ? void 0 : {
											rename: () => {
												/* v8 ignore next -- narrowing guard: the actions object exists only for real-workspace groups. */
												if (group.workspaceId !== void 0) onRenameRequest(group.workspaceId, group.label, group.cwd);
											},
											delete: () => {
												/* v8 ignore next -- narrowing guard: the actions object exists only for real-workspace groups. */
												if (group.workspaceId !== void 0) onDeleteRequest(group.workspaceId, group.label, group.cwd);
											}
										}
									}),
		...
		function FlatList({ useSessions, open, forkSession, onSessionRename, onSessionArchive, archivedSessionIds, orderBy, sessionOrderByAccount, sessionUpdatedAtByAccount, syncSessionOrderAccount, setSessionOrder, onSessionDelete, onSessionMove, t }) {
			const list = useSessions((s) => s);
		...
		*/
		function WorkspaceBrowser({ wide, expandSidebar, useSessions, useWorkspaces, useStore, actions, startSession, open, renameSession, forkSession, renameWorkspace, deleteWorkspace, insertWorkspaceBefore, archiveSession, insertSessionBefore, createWorkspace, searchSessions, searchResultLimit, useDirectoryFlow, renderSlot, deleteSession, moveSession, t }) {
			const workspaces = useWorkspaces((state) => state.items);
			const currentSessionId = useSessions((s) => s.current);
			const currentSessionBlank = useSessions((s) => s.current === void 0 ? void 0 : s.byId[s.current]?.blank);
			...
			const subWorkspacePickRef = (0, react.useRef)(false);
			...
			const confirmRename = () => {
				...
				const targetWorkspace = workspaces.find((w) => w.workspaceId === renameTarget.workspaceId);
				if (targetWorkspace?.sessionIds.includes(currentSessionId) && currentSessionBlank === false) {
					setRenameError(t("renameWorkspace.liveError"));
					return;
				}
				setRenaming(true);
				setRenameError(null);
			...
			const confirmSessionDelete = () => {
				...
				if (sessionDeleteTarget.sessionId === currentSessionId) {
					setSessionDeleteError(t("deleteSession.liveError"));
					return;
				}
			...
			const [moveSessionTarget, setMoveSessionTarget] = (0, react.useState)(null);
			const [moveFlowOpen, setMoveFlowOpen] = (0, react.useState)(false);
			const [moveBusy, setMoveBusy] = (0, react.useState)(false);
			const [moveError, setMoveError] = (0, react.useState)(null);
			const onSessionMove = (sessionId, title) => {
				setMoveSessionTarget({ sessionId, title, targetCwd: null });
				setMoveFlowOpen(true);
				setMoveError(null);
			};
			const onMoveTargetPicked = (path) => {
				setMoveFlowOpen(false);
				setMoveSessionTarget((target) => target === null ? null : { ...target, targetCwd: path });
			};
			...
			const confirmSessionMove = () => {
				if (moveBusy || moveSessionTarget === null || moveSessionTarget.targetCwd === null) return;
				if (moveSessionTarget.sessionId === currentSessionId) {
					setMoveError(t("moveSession.liveError"));
					return;
				}
				setMoveBusy(true);
				setMoveError(null);
				moveSession(moveSessionTarget.sessionId, moveSessionTarget.targetCwd).then(() => {
					setMoveBusy(false);
					setMoveSessionTarget(null);
				}).catch((reason) => {
					setMoveBusy(false);
			...
			const confirmDelete = () => {
				...
				if (targetWorkspace?.sessionIds.includes(currentSessionId) && currentSessionBlank === false) {
					setDeleteError(t("deleteWorkspace.liveError"));
					return;
				}
				setDeleting(true);
				setDeleteCommittedId(null);
				...
				children: [
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							(0, react_jsx_runtime.jsxs)("div", {
								...
								children: [wide && (0, react_jsx_runtime.jsx)(ViewOptionsMenu, {
									...
									children: (0, react_jsx_runtime.jsx)("button", {
										...
										onClick: () => {
											subWorkspacePickRef.current = false;
											setWsPickerOpen((v) => !v);
										},
							...
							(0, react_jsx_runtime.jsx)(WorkspacePickFlow, {
								...
								onPick: (workspaceId) => {
									setWsPickerOpen(false);
									if (!subWorkspacePickRef.current) startSession(workspaceId);
									subWorkspacePickRef.current = false;
								},
								onClose: () => {
									setWsPickerOpen(false);
					...
					(0, react_jsx_runtime.jsx)("div", {
						...
						children: wide && (normalizedQuery !== "" ? (0, react_jsx_runtime.jsx)(SearchResults, {
							...
							orderBy,
							onAddSubWorkspace: () => {
								subWorkspacePickRef.current = true;
								setWsPickerOpen(true);
							},
							t,
							onRenameRequest: (workspaceId, currentTitle, cwd) => {
								setRenameTarget({
									workspaceId,
									currentTitle,
									cwd
							...
							onDeleteRequest: (workspaceId, title, cwd) => {
								setDeleteTarget({
									workspaceId,
									title,
									cwd
					...
					(0, react_jsx_runtime.jsxs)(_deepseek_ai_dsh_client_ui_primitives.Modal, {
						...
						...renameTarget === null ? {} : { description: t("rename.desc", { name: renameTarget.currentTitle, cwd: renameTarget.cwd }) },
						footer: (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [(0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Button, {
							variant: "outline",
							disabled: renaming,
							onClick: closeRename,
						...
						children: [
							...
							renaming && (0, react_jsx_runtime.jsx)("div", {
								className: WorkspaceBrowser_module_css_default.deleteStatus,
								role: "status",
								children: t("rename.pending")
							}),
					...
					(0, react_jsx_runtime.jsxs)(_deepseek_ai_dsh_client_ui_primitives.Modal, {
						...
						...deleteTarget === null ? {} : { description: t("delete.desc", { name: deleteTarget.title, cwd: deleteTarget.cwd }) },
						footer: (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [(0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Button, {
							variant: "outline",
							disabled: deleting,
							onClick: closeDelete,
					...
					(0, react_jsx_runtime.jsxs)(_deepseek_ai_dsh_client_ui_primitives.Modal, {
						...
						...sessionDeleteTarget === null ? {} : { description: t("deleteSession.desc", { name: sessionDeleteTarget.title }) },
						footer: (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [(0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Button, {
							variant: "outline",
							...
							children: t("deleteSession.confirm")
						})] }),
					...
					(0, react_jsx_runtime.jsxs)(_deepseek_ai_dsh_client_ui_primitives.Modal, {
						open: moveSessionTarget !== null && moveSessionTarget.targetCwd !== null,
						onClose: closeSessionMove,
						closeLabel: t("close"),
						title: t("moveSession.title"),
						...moveSessionTarget === null || moveSessionTarget.targetCwd === null ? {} : { description: t("moveSession.desc", { name: moveSessionTarget.title, cwd: moveSessionTarget.targetCwd }) },
						footer: (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [(0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Button, {
							variant: "outline",
							disabled: moveBusy,
							onClick: closeSessionMove,
							children: t("cancel")
						}), (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Button, {
							variant: "outline",
							className: WorkspaceBrowser_module_css_default.deleteAction,
							disabled: moveBusy,
							onClick: confirmSessionMove,
							children: t("moveSession.confirm")
						})] }),
						children: [moveBusy && (0, react_jsx_runtime.jsx)("div", {
							className: WorkspaceBrowser_module_css_default.deleteStatus,
							role: "status",
							children: t("moveSession.pending")
						}), moveError !== null && (0, react_jsx_runtime.jsx)("div", {
							className: WorkspaceBrowser_module_css_default.renameError,
							role: "alert",
							children: moveError
						})]
		...
		*/
		...
		const zh = {
			...
			"rename.desc": "将把工作区重命名为新名称，并同步重命名磁盘文件夹（原路径：{cwd}）。该文件夹下的会话与子工作区会一并迁移；名称会立即更新，重启 dsh web 后同步会话数据。",
			"rename.pending": "正在重命名工作区…",
			"renameWorkspace.liveError": "不能重命名包含当前打开会话的工作区，请先切换到其他工作区。",
			"rename.session.title": "重命名会话",
			"field.workspaceName": "工作区名称",
			"field.sessionName": "会话名称",
			"delete.workspace": "删除工作区",
			"delete.desc": "将删除磁盘文件夹“{cwd}”及其全部内容，并把工作区“{name}”从列表中移除。该文件夹下的子工作区会一并删除；会话日志保留，其会话显示在“未分组”下。",
			"delete.pending": "正在删除工作区…",
			"deleteWorkspace.liveError": "不能删除包含当前打开会话的工作区，请先切换到其他工作区。",
			"menu.fork": "分叉会话",
			"menu.archiveSession": "归档会话",
			"menu.deleteSession": "删除会话",
			"deleteSession.title": "删除会话",
			"deleteSession.desc": "将永久删除会话“{name}”及其日志，无法撤销。",
			"deleteSession.confirm": "删除",
			"deleteSession.pending": "正在删除会话…",
			"deleteSession.liveError": "不能删除当前打开的会话，请先切换到其他会话。",
			"menu.moveSession": "移动会话",
			"moveSession.title": "移动会话",
			"moveSession.desc": "将把会话“{name}”移动到 {cwd}。移动后请重启 dsh web。",
			"moveSession.confirm": "移动",
			"moveSession.pending": "正在移动会话…",
			"moveSession.liveError": "不能移动当前打开的会话，请先切换到其他会话。",
		...
		const en = {
			...
			"menu.addSubWorkspace": "Add sub-workspace",
			...
			"rename.desc": "Renames the workspace and its folder on disk (current path: {cwd}). Sessions and sub-workspaces under it are migrated along; the name updates immediately, restart dsh web to resync session data.",
			"rename.pending": "Renaming workspace…",
			"renameWorkspace.liveError": "Cannot rename a workspace that contains the open session — switch to another workspace first.",
			"rename.session.title": "Rename session",
			"field.workspaceName": "Workspace name",
			"field.sessionName": "Session name",
			"delete.workspace": "Delete workspace",
			"delete.desc": "This deletes the folder “{cwd}” and all its contents, and removes the workspace “{name}” from the list. Sub-workspaces under it are deleted too; session logs are kept and their sessions appear under Ungrouped.",
			"delete.pending": "Deleting workspace…",
			"deleteWorkspace.liveError": "Cannot delete a workspace that contains the open session — switch to another workspace first.",
			"menu.fork": "Fork session",
			"menu.archiveSession": "Archive session",
			"menu.deleteSession": "Delete session",
			"deleteSession.title": "Delete session",
			"deleteSession.desc": "This permanently deletes the session “{name}” and its log. This cannot be undone.",
			"deleteSession.confirm": "Delete",
			"deleteSession.pending": "Deleting session…",
			"deleteSession.liveError": "Cannot delete the open session — switch to another session first.",
			"menu.moveSession": "Move to workspace",
			"moveSession.title": "Move session",
			"moveSession.desc": "Move session “{name}” to {cwd}. Restart dsh web afterwards.",
			"moveSession.confirm": "Move",
			"moveSession.pending": "Moving session…",
			"moveSession.liveError": "Cannot move the open session — switch to another session first.",
		...
		*/
		function apply(ctx) {
			...
			const executeHostCommand = async (commandLine) => {
				const currentId = ctx.sessions.list.getSnapshot().current;
				if (currentId === void 0) throw new Error("open a session first, then run the command");
				const session = ctx.sessions.binding(currentId)?.session;
				if (session === void 0) throw new Error("no session binding for the current session");
				const result = await session.remote.commands.execute(currentId, commandLine);
				if (!result.ok) throw new Error(result.error.message);
			};
			...
			const browserInjected = () => ({
				...
				renameWorkspace: async (workspaceId, title) => {
					await executeHostCommand(`/rename-workspace ${workspaceId} ${title}`);
				},
				deleteWorkspace: async (workspaceId) => {
					await executeHostCommand(`/delete-workspace ${workspaceId}`);
				},
				...
				deleteSession: async (sessionId) => {
					await executeHostCommand(`/delete-session ${sessionId}`);
					// The host deletes the log + workspace account, but the browser's
					// session-list store is not told about it. Re-pull the baseline so
					// the deleted session (including Ungrouped rows) disappears at once.
					try {
						await ctx.sessions.refresh();
					} catch {
						// Deletion already succeeded; a refresh failure must not surface as a false error.
					}
				},
				moveSession: async (sessionId, targetCwd) => {
					await executeHostCommand(`/move-session ${sessionId} ${targetCwd}`);
				},
