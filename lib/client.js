		...
		function apply(ctx) {
			...
			const browserInjected = () => ({
				...
				deleteSession: async (sessionId) => {
					if (ctx.remote === void 0 || ctx.remote.commands === void 0) throw new Error("remote.commands unavailable");
					const result = await ctx.remote.commands.execute(sessionId, `/delete-session ${sessionId}`);
					if (!result.ok) throw new Error(`delete-session failed: ${result.error.message}`);
				},
				moveSession: async (sessionId, targetCwd) => {
					if (ctx.remote === void 0 || ctx.remote.commands === void 0) throw new Error("remote.commands unavailable");
					const result = await ctx.remote.commands.execute(sessionId, `/move-session ${sessionId} ${targetCwd}`);
