...
async function deleteWorkspace(ctx, workspaceId) {
	...
	if (live.length > 0) {
		...
		throw new Error(`cannot delete workspace "${workspace.title}": ${live.length} open session(s) live under "${workspace.path}" — ${ids}. Close them (switch to another workspace, or restart dsh web) first.`);
	}
 ...
 * any non-blank session under the folder is live. After the migration the
 * workspace records are re-applied through the storage domain table so the
 * sidebar reflects the new name immediately (no restart required).
 *
 * @param ctx - host root context.
 * @param workspaceId - the workspace to rename.
 * @param newTitle - the new folder name / display title.
...
async function renameWorkspace(ctx, workspaceId, newTitle) {
	...
	if (live.length > 0) {
		const ids = live.map((session) => session.id).join(", ");
		throw new Error(`cannot rename workspace "${workspace.title}": ${live.length} open session(s) live under "${workspace.path}" — ${ids}. Close them (switch to another workspace, or restart dsh web) first.`);
	}
