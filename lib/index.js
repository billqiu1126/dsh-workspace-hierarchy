...
async function renameWorkspace(ctx, workspaceId, newTitle) {
	...
	if (live.length > 0) {
		...
		throw new Error(`cannot rename workspace "${workspace.title}": ${live.length} open session(s) live under "${workspace.path}" — ${described}. Restart dsh web, then rename before reopening those sessions.`);
	}
