...
async function deleteWorkspace(ctx, workspaceId) {
	...
	if (live.length > 0) {
		...
		throw new Error(`cannot delete workspace "${workspace.title}": ${live.length} open session(s) live under "${workspace.path}" — ${ids}. Restart dsh web, then delete before reopening those sessions.`);
	}
...
async function renameWorkspace(ctx, workspaceId, newTitle) {
	...
	if (live.length > 0) {
		const ids = live.map((session) => session.id).join(", ");
		throw new Error(`cannot rename workspace "${workspace.title}": ${live.length} open session(s) live under "${workspace.path}" — ${ids}. Restart dsh web, then rename before reopening those sessions.`);
	}
